...
- 怪物流水线在全部拆帧结束后会打成可下载的 zip 素材（`mediaKind=archive`，目录为 `{名称}/{actionId}/{fps}fps/*.png`，另含草稿 `sidecar.json`），落在同一素材文件夹。逐帧导出 `frames.json` 增加 `durationSeconds`。骨骼页标明「导出骨骼包」是 fbanim v2 预览而非 LIAF（LIAF 走发布 v3）。
...
- 武器配置可选择素材库里的武器图，未选真实素材时禁止保存，避免附件引用不存在的占位 ID。
- 武器外观可调图层偏移（与装备试穿相同：0 盖住手，-1 压到手下面）。
- 装备试穿预览支持像角色绑定一样在画布上拖动附件的位置、旋转、缩放与轴心，并与右侧数值同步。
- 更换装备主插槽会释放原来的占用（不再把 head 一直勾住）；附件改到其他插座时，单占用装备会自动对齐到对应插槽，避免多件装备抢同一个头槽。

### 修复

- 怪物静图去背不再调用 `ffprobe`，尺寸从 PNG 头读取；缺少 ffmpeg 时提示安装命令，而不再抛 PATH 原文。
- 怪物图生视频不再把 256 透明 PNG 直接上传给插件，改为待机图 RGB JPEG，并对参考图上传 502/503/504 重试。
- 素材库下载中文名压缩包时使用 RFC 5987 `filename*`，并由页面 fetch 落盘，避免 Chrome 提示「无法下载 / 联系你的组织」。
- 调整骨骼与绑定素材共用左右分栏：画布占左侧主区，检查器在右侧可滚动，不再把工具条撑成空白列、把数值裁在窗口外。
- 第 5 步动作制作用醒目栏显示剪辑 ID 并可一键复制（左侧每条动作旁也有复制按钮），避免和游戏动作名、第 3 步姿态配置 ID 混淆。
- 语义事件标记改为显示类型（如 `footstep.left`），不再沿用表单默认名称 `fire`；插入非开火类型时会自动改成对应名称。
- 编辑中的装备会直接显示在画布上（不必先点试穿），点击装备图即可出现和肢体一样的位移/旋转/缩放框，橙色骨骼点不再挡住点选。
- 新建武器未绑定素材库图片时会提示选择武器图，不再在保存时才报「附件引用的素材不存在」。
- 保存武器时若姿态配置 ID 尚未存在，会自动登记空姿态，不再报「stanceProfile 不存在」。
- 武器兼容预览不再默认叠上正在编辑的草稿；清空预览后画布上的武器也会消失。
- 动作配置语义事件时间轴可直接拖动，预览会采样当前剪辑姿势；关闭轨道编辑后会重新载入关键帧。
- 动作编辑弹窗把「动作时长」放在时间轴上方常驻显示，不再藏进折叠的高级设置。
- 语义事件时间轴可直接拖动预览姿势；关闭动作编辑弹窗后右侧预览会重新载入刚保存的轨道。
- 装备「替换隐藏」步可直接点选角色肢体，不必手抄槽 ID；绑定检查器也会显示肢体槽 ID。
- 装备图层可压到插座对应肢体下面（图层偏移 -1），不再永远盖住全身。
