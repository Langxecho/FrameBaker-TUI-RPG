...
已有 PNG / ZIP 的导入表单中，每个动作都可填写**受击参考点**；平A与特殊攻击还可填写**枪口 X/Y/方向**及**发射表现时刻（毫秒）**。这些数值使用画布左上为 `(0, 0)`、X 向右、Y 向下；朝向镜像时由下游按 R1 合同同时镜像图像与 anchor。

- `hit` 与 `muzzle` 会写入该动作的每一帧，适合固定挂点；需随帧移动时可通过导入 API 的 `frameAnchors` 传逐帧覆盖。
- 平A时间点导出为 `weapon.fire`，特殊攻击导出为 `effect.trigger`，两者均为 `presentationOnly: true`。
- 标记只告诉表现层何时播放枪口闪光、曳光或命中火花；它**不能**选择目标、判定命中或计算伤害。`combat.hitbox.*` 会被导出器拒绝。
- 留空允许保存尚未标注完的制作工程；坐标越出画布、同帧重复 anchor、动作末尾/之外的时刻会明确报错，不生成假成功包。

本仓已有确定性 `spark` 攻击特效笔刷可用于制作金属命中火花的源工程；其导出到正式表现资源仍等待 C-02/C-03 的资源与编排接收，不能把编辑器预览当作已完成运行时资源分发。
