...
export interface BodyPartMetadata { semantic: string; category: "base" | "replaceable" | "permanent-attachment"; hideTags: string[]; replaceable: boolean; drawGroup: string }
...
function immutable<T>(value: T): T { return Object.freeze(value) as T; }
function snapshotTransform(transform: Transform): Transform { return immutable({ translation: immutable([...transform.translation]) as Transform["translation"], rotation: immutable([...transform.rotation]) as Transform["rotation"], scale: immutable([...transform.scale]) as Transform["scale"] }); }
function snapshotAttachment(attachment: EquipmentAttachment): EquipmentAttachment { return immutable({ ...attachment, size: immutable([...attachment.size]) as EquipmentAttachment["size"], pivot: immutable([...attachment.pivot]) as EquipmentAttachment["pivot"], rest: snapshotTransform(attachment.rest) }); }
...
export function validateEquipmentDefinition(value: unknown, body: BodyProfile): ValidationResult<EquipmentDefinition> {
  ...
  if (Array.isArray(equipment.occupiedSlots) && !equipment.occupiedSlots.includes(equipment.primarySlot)) issues.push(issue("primarySlot", "主插槽必须被占用"));
  ...
  if (equipment.effectBindings) for (const [index, effect] of equipment.effectBindings.entries()) if (effect.socket !== undefined && !socketIds.has(effect.socket)) issues.push(issue(`effectBindings[${index}].socket`, "效果插座不存在"));
  if (equipment.weapon) { const weapon = equipment.weapon; validateTransform(weapon.primaryGrip, "weapon.primaryGrip", issues); if (weapon.holdMode === "two_hand" && (!weapon.secondaryGrip || equipment.occupiedSlots.length !== 2)) issues.push(issue("weapon", "two_hand 必须声明 secondaryGrip 且占用两个插槽")); if (weapon.secondaryGrip) validateTransform(weapon.secondaryGrip, "weapon.secondaryGrip", issues); if (!validId(weapon.stanceProfile)) issues.push(issue("weapon.stanceProfile", "姿态配置无效")); if (weapon.secondaryHandConstraint) { const c = weapon.secondaryHandConstraint; if (!validId(c.id) || !validId(c.upperBoneId) || !validId(c.lowerBoneId) || !validId(c.endBoneId) || !socketIds.has(c.targetSocket) || !finite(c.mix) || c.mix < 0 || c.mix > 1 || (c.stretch === "limited" && (!finite(c.maxStretch) || c.maxStretch! < 1))) issues.push(issue("weapon.secondaryHandConstraint", "IK 参数无效")); } }
...
export function validateLoadout(body: BodyProfile, definitions: readonly EquipmentDefinition[], loadout: CharacterLoadout): ValidationResult<CharacterLoadout> {
  const issues: ValidationIssue[] = []; const byId = new Map<string, EquipmentDefinition>(); const counts = new Map<string, number>(); const tags = new Set<string>();
  for (const [index, definition] of definitions.entries()) if (byId.has(definition.id)) issues.push(issue(`definitions[${index}].id`, "装备 ID 重复")); else byId.set(definition.id, definition);
  if (loadout.bodyProfileId !== body.id) issues.push(issue("bodyProfileId", "BodyProfile 不匹配"));
  const equippedIds = new Set<string>();
  for (const [index, equipped] of loadout.equipment.entries()) { const path = `equipment[${index}]`; if (equippedIds.has(equipped.equipmentId)) issues.push(issue(`${path}.equipmentId`, "装备不能重复装备")); equippedIds.add(equipped.equipmentId); const definition = byId.get(equipped.equipmentId); if (!definition) { issues.push(issue(`${path}.equipmentId`, "装备不存在")); continue; } if (equipped.primarySlot !== definition.primarySlot) issues.push(issue(`${path}.primarySlot`, "主插槽不匹配")); for (const slot of definition.occupiedSlots) { counts.set(slot, (counts.get(slot) ?? 0) + 1); if (definition.tags.some((tag) => !body.slots.find((item) => item.id === slot)?.accepts.includes(tag))) issues.push(issue(`${path}.equipmentId`, `装备标签不被插槽 ${slot} 接受`)); } for (const conflict of definition.conflictTags) if (tags.has(conflict) || loadout.equipment.some((other) => other !== equipped && byId.get(other.equipmentId)?.tags.includes(conflict))) issues.push(issue(path, "装备冲突")); for (const tag of definition.tags) tags.add(tag); }
...
export function assembleLoadout(body: BodyProfile, binding: CharacterBinding, definitions: readonly EquipmentDefinition[], loadout: CharacterLoadout): ValidationResult<AssembledLoadout> {
  ...
  const value: AssembledLoadout = { bodyProfileId: body.id, visibleParts: immutable([...visibleParts]) as string[], hiddenParts: immutable([...new Set(hiddenParts)]) as string[], attachments: immutable(attachments.map(snapshotAttachment)) as EquipmentAttachment[], occupiedSlots: immutable([...occupiedSlots].sort()) as string[], effects: immutable(effects.map((effect) => immutable({ ...effect }))) as EquipmentEffectBinding[], actionOverrides: immutable({ ...actionOverrides }), diagnostics: immutable([]) as ValidationIssue[] };
  return { ok: true, value: immutable(value), issues: [] };
}
